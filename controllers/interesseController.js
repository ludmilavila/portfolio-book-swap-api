...
exports.cadastrar = (req, res) => {
  ...
  const livro = data.livros.find(l => l.id === livroId);
  ...
  if (data.interesses.find(i => i.livroId === livroId && i.usuarioId === req.user.id)) {
  ...
  data.interesses.push(interesse);
  res.status(201).json({ message: 'Interesse cadastrado com sucesso.' });
};

exports.listar = (req, res) => {
  const meusInteresses = data.interesses.filter(i => i.usuarioId === req.user.id);
